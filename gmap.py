...
class GMap:
    ...
    def remove_dart(self, dart):
        ...
        for i in self.alphas.keys():
            self.alphas[i][self.alphas[i][dart]] = dart
            del self.alphas[i][dart]


    def is_valid(self, printError = False):
        """
        ...
        for dart, alpha_0_of_dart in self.alphas[0].items():
             if dart == alpha_0_of_dart :
                if printError : print(dart,'fixed point for alpha_0')
                return False # no fixed point
             if dart != self.alpha(0,alpha_0_of_dart) :
                if printError : print(dart,'not an involution : alpha_0(alpha_0(d)) != d')
                return False # alpha_0 is an involution

        for dart, alpha_1_of_dart in self.alphas[1].items():
             if dart == alpha_1_of_dart :
                if printError : print(dart,'fixed point for alpha_1')
                return False # no fixed point
             if dart != self.alpha(1,alpha_1_of_dart) :
                if printError : print(dart,'not an involution : alpha_1(alpha_1(d)) != d')
                return False # alpha_1 is an involution

        for dart in self.darts(): # alpha_0 alpha_2 is an involution
            if self.alpha_composed([0,2,0,2],dart) != dart:
                if printError : print(dart,'not an involution : alpha_2(alpha_2(d)) != d')
                return False
    ...
    def print_alphas(self):
        ...
        try:
            ...
            print "d     α0  α1  α2"
            for d in self.darts():
                print d," | ",Fore.MAGENTA+str(self.alpha(0,d))," ",Fore.GREEN+str(self.alpha(1,d))," ",Fore.BLUE+str(self.alpha(2,d))," ",Style.RESET_ALL , self.positions.get(d)
    ...
    def is_removable_cell(self, dart, degree):
        ...
        """
        if degree is 2: return False
        if degree is 1: return True
        ...
        if degree is 0:
            for d in self.orbit(dart, [1,2]):
                if self.alpha_composed([1,2], d) != self.alpha_composed([2,1], d): return False
    ...
    def remove_cell(self, dart, degree):
        ...
        if not self.is_removable_cell(dart, degree):
            return

        cell_orbit = self.orbit(dart, [0,2])
        for d in cell_orbit:
            n = self.alpha(degree,dart)
            if n not in cell_orbit:
                degree1 = 0 if degree == 2 else degree + 1
                np = self.alpha_composed([degree, degree1], n)
                while np in cell_orbit:
                    np = self.alpha_composed([degree, degree1], np)
                self.link_darts(degree,n,np)
